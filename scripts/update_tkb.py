...
def find_pdf_file():
    ...
    if pdfs:
        tkb_pdfs = [p for p in pdfs if 'tkb' in os.path.basename(p).lower()]
        return tkb_pdfs[0] if tkb_pdfs else pdfs[0]
...
def process_pdf(pdf_path):
    ...
    for page_num in range(len(doc)):
        ...
        print(f"[{page_num+1:02d}/{len(doc)}] Lớp {class_code:12s} -> {len(rows):2d} dòng môn học")
    ...
    # Save to data/
    for out_folder in [data_dir]:
        out_json = os.path.join(out_folder, "classes_database.json")
        out_js = os.path.join(out_folder, "classes_data.js")
    ...
    print("File classes_data.js và classes_database.json đã được lưu mới.")
    print("Bây giờ bạn chỉ cần mở trình duyệt và nhấn F5 để xem Thời Khóa Biểu mới!")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    if not pdf:
        ...
        print("Huong dan:")
        print("1. Hay copy file PDF vao cung thu muc nay, hoac")
        print("2. Keo tha file PDF vao file cap_nhat_tkb.bat")
    else:
        process_pdf(pdf)
